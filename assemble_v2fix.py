#!/usr/bin/env python3
"""Assemble Malema v2 portrait debate video - v2 fix (audio normalized)."""
...
AUDIO = f"{BASE}/audio"
IMG = f"{BASE}/img"
OUT = f"{BASE}/segments2"
FINAL = f"{BASE}/output"

os.makedirs(OUT, exist_ok=True)
os.makedirs(FINAL, exist_ok=True)

# Target audio: 48000Hz stereo AAC
AR = 48000
AC = 2
AB = "128k"
...
def add_silent_audio(input_video, output, duration):
    """Add a silent stereo audio track to a video that has none."""
    cmd = [
        "ffmpeg", "-y",
        "-i", input_video,
...
def normalize_audio(input_video, output):
    """Re-encode audio to target sample rate and channel count."""
    cmd = [
...
def portrait_speaker(input_clip, output, duration=None):
    ...
    Audio normalized to target rate/channels."""
    cmd = [
...
def title_segment(image, output, duration=4):
    """Create title card with zoom and fade. Returns video-only + needs silent audio."""
    tmp = output.replace(".mp4", "_noaudio.mp4")
    frames = duration * 30
    cmd = [
        "ffmpeg", "-y", "-loop", "1", "-i", image,
...
def sources_segment(output, duration=6):
    """Create sources screen. Returns video-only + needs silent audio."""
    tmp = output.replace(".mp4", "_noaudio.mp4")
    sources_lines = [
        "Sources",
        ...
        "Hochschild - King Leopolds Ghost",
...
# --- SEG 0: Title card (no audio -> add silent) ---
print("\n### Building title card ###")
cmd, tmp, final, dur = title_segment(f"{IMG}/title-card.jpg", f"{OUT}/seg00_title.mp4", 4)
...
# --- SEG 2: Malema freeze intro (no audio -> add silent) ---
print("\n### Building Malema intro ###")
cmd, tmp, final, dur = freeze_intro(f"{IMG}/malema-freeze.jpg", "JULIUS MALEMA", "EFF Leader", f"{OUT}/seg02_malema_intro.mp4", 3)
...
# --- SEG 4: Mashaba freeze intro (no audio -> add silent) ---
print("\n### Building Mashaba intro ###")
cmd, tmp, final, dur = freeze_intro(f"{IMG}/mashaba-freeze.jpg", "HERMAN MASHABA", "ActionSA Leader", f"{OUT}/seg04_mashaba_intro.mp4", 3)
...
run(portrait_speaker(f"{CLIPS}/mashaba-02-testing-ground.mp4", f"{OUT}/seg08_mashab2.mp4"), "Mashaba clip 2 - Testing ground")
segments.append(f"{OUT}/seg08_mashab2.mp4")
...
# --- SEG 10: Split screen (Malema top, Mashaba bottom) ---
print("\n### Building split screen ###")
split_dur = 15
# Build split screen with audio from Malema clip
run([
    "ffmpeg", "-y",
    ...
    (
        ...
        f"format=yuv420p[v];"
        f"[0:a]aresample={AR},aformat=channel_layouts=stereo,atrim=0:{split_dur},asetpts=PTS-STARTPTS[a]"
    ),
    "-map", "[v]", "-map", "[a]",
    "-c:v", "libx264", "-preset", "medium", "-crf", "23",
    "-c:a", "aac", "-b:a", AB, "-ar", str(AR), "-ac", str(AC),
...
# --- SEG 14: Sources (no audio -> add silent) ---
print("\n### Building sources screen ###")
cmd, tmp, final, dur = sources_segment(f"{OUT}/seg14_sources.mp4", 6)
...
# ============================================================
# VERIFY ALL SEGMENTS HAVE UNIFORM AUDIO
# ============================================================
print("\n### Verifying segment audio uniformity ###")
for seg in segments:
    r = subprocess.run(
    ...
    print(f"  {os.path.basename(seg)}: {r.stdout.strip()}")

# ============================================================
# CONCATENATE ALL SEGMENTS (re-encode to ensure clean merge)
# ============================================================
...
], "Concatenate segments (re-encode)")

# ============================================================
# ADD BACKGROUND MUSIC (careful mix, no loudnorm)
# ============================================================
print("\n### Adding background music ###")

video_dur = get_duration(f"{FINAL}/video_no_music.mp4")

# Mix approach: keep segment audio at full volume, add music at low volume underneath
# Use amerge instead of amix to avoid the automatic normalization
run([
    "ffmpeg", "-y",
    ...
    (
        ...
        f"volume=0.12,afade=t=out:st={video_dur-3}:d=3[music];"
...
], "Final mix with music (normalize=0)")
